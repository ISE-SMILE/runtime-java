...
public class Proxy {
    ...
    private LifeCycleJarLoader loader = null;
    ...
    public Proxy(int port) throws IOException {
        ...
        //life-cycle-methods
        this.server.createContext("/onstart", new StartHandler());
        this.server.createContext("/onpause", new PauseHandler());
        this.server.createContext("/onfinish", new FinishHandler());
            ...
            try {
                ...
                if (inputObject.has("value")) {
                    ...
                    if (message.has("main") && message.has("code")) {
                        ...
                        Path jarPath = LifeCycleJarLoader.saveBase64EncodedFile(jarIs);

                        // Start up the custom classloader. This also checks that the
                        // main method exists.
                        loader = new LifeCycleJarLoader(jarPath, mainClass);
    ...
    private  abstract class MethodHandler implements HttpHandler {

        public abstract JsonObject exec(JsonObject inputObject,HashMap<String, String> env) throws Exception;
        ...
        public void handle(HttpExchange t) throws IOException {
            ...
            try {
                ...
                JsonObject output = exec(inputObject,env);
    ...
    private class RunHandler extends MethodHandler {

        @Override
        public JsonObject exec(JsonObject inputObject, HashMap<String, String> env) throws Exception {
            return loader.invokeMain(inputObject, env);
        }
    }

    private class StartHandler extends MethodHandler {
        @Override
        public JsonObject exec(JsonObject inputObject, HashMap<String, String> env) throws Exception {
            if (!started){
                started = true;
                ...
                throw new IllegalAccessException("Cannot call onstart on the action more than once.");
            }

        }
    }

    private class PauseHandler extends MethodHandler {
        @Override
        public JsonObject exec(JsonObject inputObject, HashMap<String, String> env) throws Exception {
            throw new Exception("Not yet implemented");
        }
    ...
    private class FinishHandler extends MethodHandler {
        @Override
        public JsonObject exec(JsonObject inputObject, HashMap<String, String> env) throws Exception {
            throw new Exception("Not yet implemented");
        }
